	...
	return Controller.extend("cadastralMaintenance.view.BaseController", {
		...
		fVerifyAction: function(getOnlyAction, userAction) {
			...
			if (getOnlyAction !== true) {
				...
				else if (profile !== 0 && (oView.sViewName === 'cadastralMaintenance.view.DetailPersonalData' ||
						oView.sViewName === 'cadastralMaintenance.view.DetailDocuments' ||
						oView.sViewName === 'cadastralMaintenance.view.DetailAddress' ||
						oView.sViewName === 'cadastralMaintenance.view.DetailEducation' ||
						oView.sViewName === 'cadastralMaintenance.view.DetailBankData' ||
						oView.sViewName === 'cadastralMaintenance.view.DetailDependents' ||
						oView.sViewName === 'cadastralMaintenance.view.DetailHealth')) {
		...
		onNavBack: function() {
			...
			this.getRouter().myNavToWithoutHash({
				currentView: this.getView(),
				targetViewName: "cadastralMaintenance.view.Master",
				targetViewType: "XML",
				transition: "slide"
			});
		},
		...
		setDocumentStatus: function(reqNumber,status, doctype){
			var that = this;
			var oDialog = that.getView().byId("BusyDialog");
			  ...
			  */
			  ...
			  OData.request(f, function(data, oSuccess) {
				...
				*/
				...
				//or (var i = 0; i < anexos.length; i++) {
				  //if (anexos[i].New === true) {
          const uploadCollection = that.getView().byId("UploadCollection");
          const conta_anexos =  uploadCollection.getItems().length;
					...
					dados += "BENEFICIOS;" + doctype + ";" + req + ";STATUS;" + conta_anexos + ";" + status + ";" + that.getView().getModel("ET_HEADER").getData().PERNR;
